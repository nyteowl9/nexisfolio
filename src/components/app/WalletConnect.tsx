...
export function BrokerageImportCard() {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ ...card, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ width: 40, height: 40, borderRadius: 10, background: "var(--t-stocks)" }} />
      <div>
        <div style={{ fontSize: 14.5, fontWeight: 600, color: "var(--ink)" }}>Brokerage import</div>
        <div style={{ fontSize: 12.5, color: "var(--ink-3)", marginTop: 4, lineHeight: 1.5 }}>
          Export your positions as CSV from E-Trade / Fidelity / Schwab and drop it in — symbols, shares &amp; cost basis auto-mapped. Free, no login.
        </div>
      </div>
      <button onClick={() => setOpen(true)} style={{ marginTop: "auto", padding: "9px", textAlign: "center", background: "var(--bg-sunk)", color: "var(--ink)", border: "var(--hair) solid var(--border)", borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-sans)" }}>Import CSV</button>
      {open && <AddTradeModal onClose={() => setOpen(false)} initialMode="bulk" />}
    </div>
  );
}
