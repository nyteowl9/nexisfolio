      ...
      <div style={{ ...card, overflow: "hidden" }}>
        ...
        {conns.length === 0 ? (
          ...
          conns.map((c, i) => {
            ...
            return (
              <div key={c.id} className="nw-conn" style={{ display: "grid", gridTemplateColumns: "1fr 110px 140px 100px 170px", alignItems: "center", gap: 12, padding: "14px 22px", borderTop: i ? "var(--hair) solid var(--border)" : "none" }}>
                <div>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>{c.provider === "wallet" && (<><ResyncButton id={c.id} /><DisconnectButton id={c.id} label={c.display_name || "wallet"} /></>)}</div>
              </div>
            );
          })
        )}
